        ...
        implements GcMonitor
{
    ...
    public void start()
    {
        log.info("GcMonitor start: starts=%s, stops=%s", STARTS.incrementAndGet(), STOPS.get());
    ...
    public void stop()
    {
        log.info("GcMonitor stop: starts=%s, stop=%s", STARTS.get(), STOPS.incrementAndGet());
        for (GarbageCollectorMXBean mbean : ManagementFactory.getGarbageCollectorMXBeans()) {
            ObjectName objectName = mbean.getObjectName();
            try {
                ...
                log.warn(e, "Failed to remove GC listener");
            }
        }
    }
